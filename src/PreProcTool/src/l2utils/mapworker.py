...
class MapWorker(object):
    ...
    def openInputDataset(self, in_fn):
        try:
            ...
            self.in_gt = [float(self.geoExtent['ulx']),
                          (float(self.geoExtent['lrx']) - float(self.geoExtent['ulx'])) / self.in_ds.RasterXSize, 0,
                          float(self.geoExtent['uly']), 0,
                          (float(self.geoExtent['lry']) - float(self.geoExtent['uly'])) / self.in_ds.RasterYSize]
    ...
    def createOutputDataset(self):
        try:
            ...
            if self.scaleType == 'nominal':
                statsDict['minMaxMasked'] = [np.asscalar(np.amin(data_test_masked)),
                                             np.asscalar(np.amax(data_test_masked))]
            ...
            for i in range(0, self.in_ds.RasterYSize, self.in_yBlockSize):
                ...
                for j in range(0, self.in_ds.RasterXSize, self.in_xBlockSize):
                    ...
                    data = np.add(data, 2147483648, dtype='uint32', casting="unsafe")
