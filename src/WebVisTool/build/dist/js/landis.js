...
(function ($) {
  $.landisMaps = function () {
    ...
    this.updateMinMax = function (mapGroupId) {
      var i, j, m, unit, rMaps = [],
        legend, filterMin, filterMax, minMax;
      for (i = 0; i < this.mapGroups.length; i++) {
        if (this.mapGroups[i].rasterMapGroup.rastermapgroup('getMapGroupId') == mapGroupId) {
...
(function ($, window, document, undefined) {
  ...
  $.widget('landis.timeControl', {
      ...
      self._playbackSpeedSlider = $('<div>')
        ...
        .slider({
          ...
          slide: function (event, ui) {
            ...
            self._playbackSpeedLabel.text('playback speed: ' + self.options.playbackSpeed.toFixed(2) + 'x');
            self._playbackTimer.set({
              time: parseInt(1000 / playbackspeed)
            });
          }
        })
        .appendTo(self._playbackPreferencesDialogContent);
    ...
    _playbackPreferencesDialogTemplate: '<span>Playback Speed: </span><br /> \
            '
...
(function ($, window, document, undefined) {
  ...
  $.widget('landis.scenarioSelector', {
    ...
    _destroy: function () {
      ...
      self.element.empty();
      $('#dropdown-scenario-selector').remove();
      //$('input[name="scenario-checkbox"]:checkbox').unbind('change');
      self._super();
    },

    _setOption: function (key, value) {
      console.log('_setOption', 'key: ', key, 'value: ', value);
      var self = this,
        prev = this.options[key],
        fnMap = {
  ...
  $.widget('landis.rastermapgroup', {
    ...
    generateHandles: function () {
      ...
      for (i = 0; i < self.stats.classification.classes.length; i++) {
        h.push({
          value: self.stats.classification.classes[i],
          color: c[i]
        });
      }
      //console.log(h)
  ...
  $.widget("landis.mapLegend", {
    ...
    _setupUserInerfaceAndEvents: function () {
      ...
      if (self._sliderMode) {
        ...
        self._rangeSlider = $('<div>').insertBefore($(self._colorSlider)).css({
          'height': '202px',
          'float': 'left',
          'margin-right': '10px'
        });
    ...
    _addHandle: function (idx) {
      ...
      handleContainer = $('<div>')
        ...
        .offset({
          top: self._value2position(self.privateHandles[idx].value),
          left: self._colorSlider.width()
        })
  ...
  $.widget('landis.chart', {
    ...
    _create: function () {
      ...
      self._flotOptions = {
        ...
        colors: ["#4067cb", "#d23d20", "#f79a1d", "#329518", "#920b99", "#3699c5",
        "#d44779", "#6ea905", "#87574d", "#7f7f7f"],
        shadowSize: 0
      };
      ...
      self._filterFieldList.on('change', function () {
        ...
        for (i = 0; i < self.options.scenarios.length; i++) {
          ...
          data = self.generateFlotData(
            [self._results[i]], [self.options.fieldName],
            'Time', {
              'EcoregionName': ['inactiv', 'inactive'],
              'Ecoregion': ['inactiv', 'inactive']
            }, {},
            'NumSites', 1);
          //console.log(self._results[i]);
          self._flotData.push({
            label: landisMetadata.getScenarioAttributeById(self.options.scenarios[i], 'name'),
            data: data
          });
    ...
    loadData: function () {
      ...
      for (s in self.options.scenarios) {
        ...
        $.get(csvFile).always(function (data2) {
          ...
          data = self.generateFlotData([results], [self.options.fieldName], 'Time', {
            'EcoregionName': ['inactiv', 'inactive'],
            'Ecoregion': ['inactiv', 'inactive']
          }, {}, 'NumSites', cf);
          self._flotData.push({
            label: landisMetadata.getScenarioAttributeById(self.options.scenarios[s], 'name'),
            data: data
          });
          self.drawChart();
    ...
    generateFlotData: function (csvObjectsByScenarios, parameterList, groupByParameter, excludeParameterValue, filterParameterValue, numSitesFieldName, conversionFactor) {
      ...
      var charts = [],
        p, chart, s, dataseries, dataseriesArr, filtering, r, exclude, include, pVal, numSitesVal, totalNumSites;
      // loop paramter List
    ...
    drawChart: function () {
      ...
      self._flot.setCrosshairTime({
        x: $('#TimeControl').timeControl('option', 'currentTime'),
        y: -1
      });
    },
...
(function ($, window, document, undefined) {
  ...
  $.widget('landis.rastermap', {
    ...
    _setupDataLayer: function () {
      var self = this,
        interval, time, i, colorRamp, dataTypedArray;
    ...
    updateDataLayer: function () {
      var self = this,
        timeMax, interval, time, isPlaying, help, layers, toShow, toHide;
...
var T_CURRENTTIME = 'currentTime',
  ...
  changeScenarios = function (topic, data) {
    ...
    var scenarioSelector = $('#ScenarioSelector'),
      ...
      i, pLimit;
    console.log('selectedScenarioIds: ', selectedScenarioIds);
  ...
  resetScenarios = function () {
    $('input[name="scenario-checkbox"]:checkbox').prop('checked', false);
    $('input[name="scenario-checkbox"]:checkbox').first().prop('checked', true);
    $('#ScenarioSelector').scenarioSelector('option', 'selectedScenarioIds', [1]);
    $('#ScenarioSelector').scenarioSelector('option', 'scenarioMax', 4);
    $('#ScenarioSelector').scenarioSelector('option', 'scenarioMin', 1);
    changeScenarios();
    var timeControl = $('#TimeControl');
    timeControl.timeControl('stopPlayback');
    timeControl.timeControl('option', 'currentTime', 0);
    timeControl.timeControl('option', 'playbackSpeed', 1);
    timeControl.timeControl('option', 'snapToInterval', false);
    timeControl.timeControl('option', 'snapToInterval', false);
    timeControl.timeControl('option', 'timeMin', 0);
    timeControl.timeControl('option', 'timeMax', 100);
    timeControl.timeControl('option', 'timeInterval', 1);
  ...
  resetMaps = function () {
    var scenarioSelector = $('#ScenarioSelector');
    $('#MapSelector').mapSelector('destroy');
    var sLimit = 4;

    var mapSelector = $('#MapSelector').mapSelector({
      mapoutputs: landisMetadata.publishMapOutputs(),
      hiddenUIContainer: '#HiddenUIContainer'
    });
    mapSelector.mapSelector('option', 'selectedParameterIds', []);
    mapSelector.mapSelector('option', 'mapoutputs', {});
    mapSelector.mapSelector('option', 'selectedMapId', 0);
    mapSelector.mapSelector('option', 'parameterLimit', 4);
  ...
  resetCharts = function () {
    var chartSelector = $('#ChartSelector').chartSelector('destroy');
    $('#ChartSelector').chartSelector({
      chartoutputs: landisMetadata.publishTableOutputs(),
      hiddenUIContainer: '#HiddenUIContainer'
    });
    chartSelector.chartSelector('option', 'chartoutputs', {});
    chartSelector.chartSelector('option', 'selectedParameterIds', []);
    chartSelector.chartSelector('option', 'parameterLimit', 4);
...
window.onload = function () {
  ...
  $('#view-instructions').on('click', function() {
    $('#splash').css('display', 'none');
